...
module.exports = async function instrument (tracker, logger, { repl, instrumentPort, instrumentHost, instrumentDetailed }) {
  const server = fastify({ logger })

  const replSeed = repl ? setupRepl({ tracker }) : null
  const instrumentedSwarm = new InstrumentedSwarm(tracker.swarm, { server })
  const hypermetrics = new Hypermetrics(promClient, { detailed: instrumentDetailed })
